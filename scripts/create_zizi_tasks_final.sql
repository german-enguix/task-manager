-- Script FINAL para crear tareas de Zizi con sistema de evidencias
-- Versión corregida que se adapta a la estructura real de la base de datos
-- Ejecutar en el panel SQL de Supabase

-- ===============================
-- 0. DIAGNÓSTICO: VERIFICAR ESTRUCTURA REAL
-- ===============================
...
SELECT 'ESTRUCTURA DE TABLA SUBTASKS:' as info;
SELECT column_name, data_type, is_nullable 
FROM information_schema.columns 
...
-- ===============================
-- 2. VERIFICAR Y OBTENER USUARIOS
-- ===============================
...
-- Verificar y obtener IDs de usuarios existentes
-- Método robusto: JOIN entre auth.users y profiles
...
IF zizi_id IS NULL THEN
    RAISE EXCEPTION 'Usuario zizi@taskmanager.com no encontrado. Verifica que existe en auth.users y tiene perfil en profiles';
END IF;

IF german_id IS NULL THEN
    RAISE EXCEPTION 'Usuario german@taskmanager.com no encontrado. Verifica que existe en auth.users y tiene perfil en profiles';
END IF;

IF albert_id IS NULL THEN
    RAISE EXCEPTION 'Usuario albert@taskmanager.com no encontrado. Verifica que existe en auth.users y tiene perfil en profiles';
END IF;
...
-- Subtareas para Tarea 1 (CON columna order_index)
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task1_id, 'Limpieza de escritorios y superficie', 'Limpiar y desinfectar todos los escritorios', false, NOW());
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task1_id, 'Aspirado y fregado de suelos', 'Aspirar alfombras y fregar suelos de todas las oficinas', false, NOW());
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task2_id, 'Configurar disposición de mesas', 'Organizar mesas en forma de U para 30 personas', false, NOW());
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task2_id, 'Probar equipo audiovisual', 'Verificar funcionamiento de proyector, micrófono y sistema de sonido', false, NOW());
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task3_id, 'Verificar destinatarios', 'Confirmar que todos los paquetes tienen destinatario válido', false, NOW());
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task4_id, 'Documentar estado de equipos', 'Registrar estado actual y necesidades de cada equipo', false, NOW());
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task5_id, 'Clasificar materiales por categorías', 'Separar y agrupar materiales similares', false, NOW());
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task5_id, 'Actualizar sistema de inventario', 'Registrar todos los cambios en el sistema digital', false, NOW());
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task6_id, 'Verificar reservas de catering', 'Confirmar menú y horarios con proveedor de catering', false, NOW());
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task7_id, 'Verificar salidas de emergencia', 'Comprobar que todas las salidas estén despejadas y señalizadas', false, NOW());
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task7_id, 'Inspección final de limpieza', 'Verificar que todas las áreas cumplan estándares de limpieza', false, NOW());
...
subtask_id := gen_random_uuid();
INSERT INTO subtasks (id, task_id, title, description, is_completed, created_at) VALUES
(subtask_id, task8_id, 'Coordinar recogida con empresa externa', 'Confirmar horarios y tipos de residuos con la empresa de reciclaje', false, NOW());
